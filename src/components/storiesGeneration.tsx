...
const StoriesGeneration = () => {
  const { error, data } = useQuery('example-request', getExample);
  const cardRefs = useRef<(HTMLDivElement | null)[]>(new Array(3).fill(null));

  if (error) return <Bomb />;
  ...
  const images = [
    { url: 'https://s1.1zoom.me/b4952/517/Fast_food_Hamburger_Buns_Vegetables_Meat_products_531239_1080x1920.jpg' },
    { url: 'https://www.receiteria.com.br/wp-content/uploads/comidas-para-ano-novo-10.png' },
    { url: 'https://stories.cybercook.com.br/wp-content/uploads/2022/09/Design-sem-nome-12.jpg' },
      ...
      <CardContainer>
        {images.map((image, index) => (
          <CardCustom key={index} ref={(el) => (cardRefs.current[index] = el)} onClick={() => handleSaveImage(index)}>
            ...
            <img className="restaurant-logo" alt="imagem" src={restaurantLogo}></img>
...
export default StoriesGeneration;
